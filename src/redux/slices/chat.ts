...
// utils
import { apiAuth, apiCommunication } from '../../utils/axios';
// @types
...
export function getContacts() {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const response = await apiAuth.get('/api/chat/contacts');
...
export async function updateMessage(message: any) {
  const response = await apiCommunication.put('/message/update', message);
  return response.data.data;
}

export async function updateChat(update: { chat_id?: string; closed_by: string }) {
  const response = await apiCommunication.put('/chat/update', update);
  return response.data.data;
}

export async function createChat(create: { order_id?: string | null; members: string[] }) {
  const response = await apiCommunication.post('/chat/create', create);
  return response.data.data;
}
...
export function getConversation(conversationKey: string) {
  return async (dispatch: Dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await apiAuth.get('/api/chat/conversation', {
...
export function markConversationAsRead(conversationId: string) {
  return async (dispatch: Dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      await apiAuth.get('/api/chat/conversation/mark-as-seen', {
...
export function getParticipants(conversationKey: string) {
  return async (dispatch: Dispatch) => {
    dispatch(slice.actions.startLoading());
    try {
      const response = await apiAuth.get('/api/chat/participants', {
        params: { conversationKey },
      });
