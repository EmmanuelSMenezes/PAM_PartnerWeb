...
export function SettingsProvider({ children }: SettingsProviderProps) {
  ...
  const getPlataformStyle = async () => {
    try {
      await getPartnerStyle(partnerId?.admin_id);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const generalStyle =
    typeof window !== 'undefined' ? localStorage.getItem('GeneralStyle') : undefined;
  const newGeneralStyle: ColorVariants | undefined = generalStyle
    ? JSON.parse(generalStyle)
    : undefined;

  let currentTheme: any = useCallback(() => newGeneralStyle || presets[0], [newGeneralStyle]);

  useEffect(() => {
    // if (newGeneralStyle !== null) {
